...
Foram modeladas apenas as três entidades estritamente necessárias para
representar o fluxo de uma conversa de chatbot. Entidades adicionais
(ex.: catálogo de produtos, atendentes humanos, tabela de "intents"
configurável) foram deliberadamente deixadas de fora por não haver ainda
uma regra de negócio definida para elas - ver "PENDENTE DE DEFINIÇÃO COM O
CLIENTE" abaixo.

### `customers`

Representa quem está conversando com o chatbot. O telefone é o dado
confiável vindo do WhatsApp, por isso é único e obrigatório; o nome é
opcional (nem sempre disponível no primeiro contato).
...
mensagem do mesmo cliente reaproveita a conversa `OPEN` mais recente; se
não houver nenhuma em aberto, uma nova é criada.
...
**PENDENTE DE DEFINIÇÃO COM O CLIENTE**: regra de quando uma conversa deve
ser encerrada automaticamente (ex.: X horas de inatividade) e se o
`context` precisa evoluir para algo mais estruturado (uma máquina de
estados) conforme os fluxos de negócio reais forem definidos.
...
pertence a exatamly um cliente. Uma conversa tem várias mensagens; cada
mensagem pertence a exatamente uma conversa.

## Migrations (Flyway)

O schema é versionado com Flyway (`src/main/resources/db/migration`).
`V1__init.sql` cria as três tabelas acima com suas chaves estrangeiras e
índices (`conversation_id` em `messages`, `customer_id`/`status` em
`conversations`). O Hibernate roda em modo `validate`: ele **não** gera
...
Novas mudanças de schema devem ser feitas criando um novo arquivo
`V2__descricao.sql` (nunca editando uma migration já aplicada).
